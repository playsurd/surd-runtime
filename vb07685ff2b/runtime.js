(function () {
  ...
  function lookupBlob(url) {
    ...
    for (var key in blobStore) if (url.indexOf(key) >= 0) return blobStore[key];
    return null;
  }
  var made = [];
  function asset(role) {
  ...
  try {
    ...
    var gpOk = false;
    try { nav.getGamepads && nav.getGamepads(); gpOk = true; } catch (e) { gpOk = false; }
    if (!gpOk) {
      try { nav.getGamepads = function () { return []; }; } catch (e) {
        try { Object.defineProperty(nav, 'getGamepads', { value: function () { return []; }, configurable: true }); } catch (e2) {}
      }
    }
  } catch (e) {}
  (function () {
    ...
    for (var name of ['localStorage', 'sessionStorage']) {
      var ok = false;
      try { var probe = window[name]; probe.setItem('__surd', '1'); probe.removeItem('__surd'); ok = true; } catch (e) { ok = false; }
      if (!ok) {
        try { Object.defineProperty(window, name, { value: memStore(), configurable: true }); post('warn', name + ' unavailable here — using an in-memory stand-in (progress will not persist)'); }
        catch (e) {}
      }
    }
  })();
