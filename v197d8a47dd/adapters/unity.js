...
window.SURD_ADAPTERS.unity = async function (M, S) {
  ...
  if (cfg.era === 'modern') {
    ...
    try {
      createUnityInstance(canvas, uconf, function (p) { S.progress(0.15 + p * 0.85); })
        .then(function (inst) { window.__surdUnity = inst; S.done(); S.post('info', 'unity instance running'); })
  ...
  if (typeof window.UnityProgress !== 'function') window.UnityProgress = function () {};
  await S.loadScript(S.url(cfg.loaderJs));
  S.progress(0.15);
  var grab = S.opaqueOrigin && S.assetLocal ? S.assetLocal : S.asset;
  var data = await grab('data');
  ...
  if (cfg.variant === 'asm') {
    legacy.asmCodeUrl = await grab('code');
    legacy.asmFrameworkUrl = await grab('framework');
    legacy.codeUrl = legacy.asmCodeUrl;
    if (M.files.memory) { var mem = await grab('memory'); legacy.asmMemoryUrl = mem; legacy.memUrl = mem; }
  } else {
    legacy.wasmCodeUrl = await grab('code');
    legacy.wasmFrameworkUrl = await grab('framework');
  }
  S.status('starting Unity game');
  ...
  var tries = 0, iv = setInterval(function () {
    ...
    if (c) {
      ...
      if ((c.width > 1 && !(c.width === 300 && c.height === 150)) || (r.width > 50 && r.height > 50)) {
        S.done(); S.post('info', 'unity instance running'); clearInterval(iv); return;
      }
    }
    if (++tries > 120) {
      clearInterval(iv);
