(function () {
  ...
  function assetLocal(role) {
    var a = M.files[role];
    if (!a) return Promise.reject(new Error('manifest missing file "' + role + '"'));
    if (a.parts && a.parts.length) return asset(role);
    return blobify(M.base + a.path, a.mime || 'application/octet-stream');
  }
  window.SURD = { M: M, status: status, progress: progress, done: done, fail: fail,
                  post: post, asset: asset, assetLocal: assetLocal, blobify: blobify,
                  loadScript: loadScript, url: function (p) { return M.base + p; },
                  opaqueOrigin: !window.isSecureContext };
