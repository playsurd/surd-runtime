  ...
  if (cfg.variant !== 'asm') {
    opts.Module = {
      locateFile: function (f) {
        if (f === 'build.wasm' && this.wasmCodeUrl) return this.wasmCodeUrl;
        return 'Build/' + f;
      },
    };
  }
  var call = {
    object: function () { opts.url = jsonUrl; UnityLoader.instantiate('gameContainer', legacy, opts); },
    url: function () { UnityLoader.instantiate('gameContainer', jsonUrl, opts); },
  };
