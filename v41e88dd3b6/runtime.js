(function () {
  ...
  var elBar, elStatus, elChips, elNote;
  function ui() {
    if (elBar) return;
    ...
    d.innerHTML =
      ...
      '<div class="surd-chips" style="display:flex;gap:3px;max-width:min(260px,60vw);flex-wrap:wrap;'
      + 'justify-content:center;align-content:center;height:26px;opacity:0;transition:opacity .3s"></div>' +
      '<div style="font:12px ui-monospace,monospace;color:#9a9aa2;min-height:16px;transition:opacity .2s">preparing</div>' +
      '<div class="surd-note" style="font:12px/1.5 system-ui,sans-serif;color:#6f6f78;max-width:min(300px,72vw);'
      + 'text-align:center;height:34px;opacity:0;transition:opacity .45s"></div>';
    ...
    elChips = d.querySelector('.surd-chips'); elNote = d.querySelector('.surd-note');
    elBar = d.querySelector('span');
    elStatus = elNote.previousElementSibling;
    d.id = 'surd-boot';
  }
  function note(msg) {
    ui();
    if (!elNote || elNote.textContent === msg) return;
    elNote.textContent = msg;
    elNote.style.opacity = msg ? 1 : 0;
  }
  function status(msg) { ui(); if (elStatus) elStatus.textContent = msg; lastStage = msg; post('info', msg); }
  function progress(f) { ui(); if (elBar) elBar.style.width = Math.max(0, Math.min(1, f)) * 100 + '%'; }
  var STALL_MS = 15000, HARD_MS = 180000;
  var FRAME_WAIT_MS = 15000, FPS_TICKS_NEEDED = 2, FRAMES_PER_TICK = 6;
  var DRAWS_NEEDED = 30;
  var net = { inflight: 0, finished: 0, lastEventAt: 0 };
  var frames = 0, draws = 0;
  try {
    var nraf = window.requestAnimationFrame && window.requestAnimationFrame.bind(window);
    if (nraf) {
      window.requestAnimationFrame = function (cb) {
        return nraf(function (t) { frames++; return cb(t); });
      };
    }
  } catch (e) {  }
  ...
  function done() {
    ...
    var lastFrames = frames, smoothTicks = 0;
    var iv = setInterval(function () {
      var now = Date.now();
      var idleFor = now - (net.lastEventAt || engineUpAt);
      var thisTick = frames - lastFrames;
      lastFrames = frames;
      smoothTicks = thisTick >= FRAMES_PER_TICK ? smoothTicks + 1 : 0;
      var needsDraws = false;
      try {
        var cs = document.querySelectorAll('canvas');
        for (var ci = 0; ci < cs.length; ci++) if (/webgl/i.test(cs[ci].__surdCtxType || '')) { needsDraws = true; break; }
      } catch (e) {  }
      var drawing = smoothTicks >= FPS_TICKS_NEEDED && (!needsDraws || draws >= DRAWS_NEEDED);
      if (!drawing) {
        if (net.inflight > 0 && idleFor < STALL_MS && now - engineUpAt < HARD_MS) {
          status('still loading game data (' + net.inflight + ' file'
            + (net.inflight === 1 ? '' : 's') + ' left, ' + net.finished + ' done)');
          return;
        }
        if (now - engineUpAt < FRAME_WAIT_MS) { status('waiting for the game to draw'); return; }
        post('warn', 'no steady rendering after ' + Math.round((now - engineUpAt) / 1000)
          + 's; showing the game anyway');
      }
      clearInterval(iv);
      lift();
  ...
  function chipsClear() {
    if (!elChips) { chipEls = []; return; }
    var row = elChips;
    row.style.opacity = 0;
    var mine = chipEls;
    chipEls = [];
    setTimeout(function () {
      if (chipEls === mine || chipEls.length === 0) row.innerHTML = '';
    }, 320);
  }
  var blobStore = {};
  var pathBlobs = {};
  var roleBlobs = {};
  function roleBlobFor(url) {
    try {
      if (typeof url !== 'string') return null;
      var q = url.split('#')[0].split('?')[0];
      if (q.indexOf('blob:') === 0 || q.indexOf('data:') === 0) return null;
      var base = q.split('/').pop().toLowerCase();
      if (/\.wasm$/.test(base)) return roleBlobs.code || null;
      if (/\.data$/.test(base)) return roleBlobs.data || null;
      if (/\.mem$/.test(base)) return roleBlobs.memory || null;
      return null;
    } catch (e) { return null; }
  }
  function pathBlobFor(url) {
    if (typeof url !== 'string' || !url) return null;
    var q = url.split('#')[0].split('?')[0];
    if (pathBlobs[q]) return pathBlobs[q];
    var shipped = false;
    try {
      for (var sp in pathBlobs) if (sp.split('/').pop() === q.split('/').pop()) shipped = true;
    } catch (e) {}
    for (var k in pathBlobs) {
      var tail = k.slice(M.base.length);
      if (tail && q.length > tail.length && q.slice(-(tail.length + 1)) === '/' + tail) return pathBlobs[k];
    }
    return shipped ? null : roleBlobFor(q);
  }
  ...
  function finishAsset(a, blob) {
    var keep = function (b, changed) {
      var u = URL.createObjectURL(b);
      if (a && a.path && changed) pathBlobs[M.base + a.path] = b;
      try {
        for (var rk in (M.files || {})) if (M.files[rk] && M.files[rk].path === a.path) roleBlobs[rk] = b;
      } catch (e) {}
      made.push(u); return rememberBlob(u, b);
    };
    var reassembled = !!(a.parts || []).length;
    if (typeof DecompressionStream === 'undefined') return Promise.resolve(keep(blob, reassembled));
    return blob.slice(0, 2).arrayBuffer().then(function (head) {
      var b = new Uint8Array(head);
      if (b.length < 2 || b[0] !== 0x1f || b[1] !== 0x8b) return keep(blob, reassembled);
      ...
      return new Response(blob.stream().pipeThrough(new DecompressionStream('gzip'))).blob()
        .then(function (out) {
          return out.slice(0, 4).arrayBuffer().then(function (h) {
            ...
            return keep(type ? new Blob([out], { type: type }) : out, true);
          }).catch(function () { return keep(out, true); });
        })
        .catch(function () { return keep(blob, reassembled); });
    }).catch(function () { return keep(blob, reassembled); });
  }
  ...
  function fetchRetry(url, opts, tries) {
    ...
    function once(target) {
      return fetch(target, opts).then(function (r) {
        if (r.ok) return r;
        var e = new Error(r.status + ' ' + target);
        e.status = r.status;
        throw e;
      });
    }
    function go() {
      attempt++;
      var useAlt = alt && attempt % 2 === 0;
      return once(useAlt ? alt : url)['catch'](function (e) {
        var retryable = !e.status || TRANSIENT[e.status];
        if (attempt >= tries || (!retryable && !alt)) throw e;
        if (!retryable && attempt > 1) throw e;
        var wait = Math.min(8000, 400 * attempt * attempt);
        post('warn', (e.status || 'network') + ' on ' + target(useAlt) + ', retrying (' + attempt + '/' + tries + ')');
        return new Promise(function (r) { setTimeout(r, wait); }).then(go);
      });
    }
    function target(useAlt) { return (useAlt ? 'mirror' : 'cdn') + ':' + url.split('/').pop(); }
    return go();
  }
  function fetchTracked(url, opts, onProgress) {
    return fetchRetry(url, opts).then(function (r) {
  ...
  function asset(role) {
    ...
    function gather(attempt) {
      ...
      var LANES = 6, next = 0;
      function lane() {
        if (next >= nParts) return Promise.resolve();
        var i = next++;
        return fetchTracked(url + a.parts[i] + bust, attempt ? { cache: 'reload' } : undefined, function (got, total) {
      ...
      var lanes = [];
      for (var li = 0; li < Math.min(LANES, nParts); li++) lanes.push(lane());
  ...
  {
    var nf0 = window.fetch && window.fetch.bind(window);
    if (nf0) {
      window.fetch = function (input, init) {
        var u = typeof input === 'string' ? input : (input && input.url) || '';
        var pb = pathBlobFor(u);
        if (pb) {
          return Promise.resolve(new Response(pb, { status: 200, headers: pb.type ? { 'Content-Type': pb.type } : undefined }));
        }
        return nf0.apply(this, arguments);
      };
    }
    try {
      var XP0 = window.XMLHttpRequest.prototype, no0 = XP0.open, ns0 = XP0.send;
      XP0.open = function (method, url) {
        this.__surdPathBlob = pathBlobFor(url);
        return no0.apply(this, arguments);
      };
      XP0.send = function () {
        var pb = this.__surdPathBlob;
        if (!pb) return ns0.apply(this, arguments);
        var self = this;
        ...
        reader.onload = function () {
          ...
          if (self.onprogress) { try { self.onprogress({ loaded: pb.size, total: pb.size, lengthComputable: true }); } catch (e) {} }
          if (self.onreadystatechange) { try { self.onreadystatechange(); } catch (e) {} }
          if (self.onload) { try { self.onload({ target: self }); } catch (e) {} }
        };
        reader.onerror = function () { if (self.onerror) self.onerror(reader.error); };
        if (self.responseType === 'text' || self.responseType === '') reader.readAsText(pb);
        else reader.readAsArrayBuffer(pb);
      };
    } catch (e) { post('warn', 'original-path XHR shim unavailable: ' + (e && e.message || e)); }
  }
  if (!window.isSecureContext) {
  ...
  try {
    ...
    CP.getContext = function (type) {
      ...
      try {
        if (ctx && /webgl/i.test(String(type)) && !ctx.__surdCounted) {
          ctx.__surdCounted = true;
          ['drawArrays', 'drawElements', 'drawArraysInstanced', 'drawElementsInstanced'].forEach(function (fn) {
            if (typeof ctx[fn] !== 'function') return;
            var orig = ctx[fn];
            ctx[fn] = function () {
              draws++;
              if (draws > DRAWS_NEEDED * 4) ctx[fn] = orig;
  ...
  (function () {
    var totalBytes = 0;
    try {
      for (var k in (M.files || {})) totalBytes += (M.files[k] || {}).bytes || 0;
      var pfst = window.SURD_PREFETCH;
      if (pfst && pfst.bytes) totalBytes += pfst.bytes;
    } catch (e) {}
    var MB = totalBytes / 1048576;
    var BIG_MB = 400;
    var size = function (mb) { return mb >= 1024 ? (mb / 1024).toFixed(1) + ' GB' : Math.round(mb) + ' MB'; };
    var startedAt = Date.now();
    if (MB >= BIG_MB) {
      note('Big game (' + size(MB) + '). First load is slow, later loads are faster.');
    }
    var iv = setInterval(function () {
      if (lifted) { clearInterval(iv); return; }
      var secs = Math.round((Date.now() - startedAt) / 1000);
      if (MB >= BIG_MB) return;
      if (secs >= 45) note('Still going. Do not reload, that restarts the download.');
      else if (secs >= 12) note('Still working, not frozen.');
    }, 1000);
  })();
  ...
  var lastStage = 'preparing';
  window.addEventListener('error', function (e) {
    if (e && e.target && (e.target.src || e.target.href)) { post('err', 'failed to load ' + (e.target.src || e.target.href)); return; }
    var msg = (e && e.message) || 'error';
    var where = '';
    if (e && e.filename) {
      where = ' at ' + String(e.filename).split('/').pop() + (e.lineno ? ':' + e.lineno : '') + (e.colno ? ':' + e.colno : '');
    }
    var stack = e && e.error && e.error.stack ? String(e.error.stack).split('\n').slice(0, 3).join(' | ') : '';
    if (/^script error\.?$/i.test(msg) && !where && !stack) {
      post('err', 'the game threw an error the browser will not describe (a cross-origin script '
        + 'without CORS headers hides the message). Last stage reached: ' + lastStage);
      return;
    }
    post('err', msg + where + (stack ? ' | ' + stack : ''));
  }, true);
  window.addEventListener('unhandledrejection', function (e) {
    var r = e && e.reason;
    var detail = r && (r.stack ? String(r.stack).split('\n').slice(0, 3).join(' | ') : (r.message || r));
    post('err', 'promise rejected: ' + (detail || '?'));
  });
})();