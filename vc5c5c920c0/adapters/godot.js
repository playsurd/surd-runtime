...
window.SURD_ADAPTERS.godot = async function (M, S) {
  ...
  /* Godot installs a service worker to add COOP/COEP headers, which only matters for
   * threaded (SharedArrayBuffer) builds. Games run from a blob: document whose scope is
   * the CDN, so the registration always fails noisily. Off unless threads are in play. */
  if (!gc.threads && !gc.godotThreads) gc.ensureCrossOriginIsolationHeaders = false;
