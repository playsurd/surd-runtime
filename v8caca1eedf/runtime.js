(function () {
  ...
  (function () {
    ...
    if (MB >= 120) {
      note('This is a big game — about ' + (MB >= 1024 ? (MB / 1024).toFixed(1) + ' GB' : Math.round(MB) + ' MB')
        + ' to download. It can take a few minutes the first time; after that it loads from your cache.');
    }
    var iv = setInterval(function () {
      if (lifted) { clearInterval(iv); return; }
      var secs = Math.round((Date.now() - startedAt) / 1000);
      if (MB >= 120) return;
      if (secs >= 45) note('Still going. Big games can take a while — please do not reload, that starts the download again.');
      else if (secs >= 12) note('Still working — this is not frozen.');
    }, 1000);
  })();
